...
describe('AchievementDirect', async () => {
  ...
  test('direct-list-achievement', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-achievement', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['SHIKIMORI_TEST_ACHIEVEMENT_ENTID']

...
//     (\`new ShikimoriSDK(...)\` / \`ShikimoriSDK.test(...)\` become
//     \`ShikimoriSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"achievement":{"test01":{"id":"test01"}},"anime":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ShikimoriSDK(...)\` and any
// \`ShikimoriSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ShikimoriSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ShikimoriSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

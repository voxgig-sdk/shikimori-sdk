...
class ShikimoriTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ShikimoriTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ShikimoriTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ShikimoriTestRunner.getenv("SHIKIMORI_TEST_LIVE")
        override = ShikimoriTestRunner.getenv("SHIKIMORI_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ShikimoriTestRunner.getenv(key)
        ...
        explain = ShikimoriTestRunner.getenv("SHIKIMORI_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["SHIKIMORI_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if ShikimoriTestRunner._test_control is not None:
            return ShikimoriTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ShikimoriTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ShikimoriTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ShikimoriTestRunner.load_test_control()
...
def load_env_local():
    ShikimoriTestRunner.load_env_local()


def env_override(m):
    return ShikimoriTestRunner.env_override(m)


def entity_data(v):
    return ShikimoriTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ShikimoriTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ShikimoriTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ShikimoriTestRunner.load_test_control()


def live_client_options():
    return ShikimoriTestRunner.live_client_options()


def live_delay_ms():
    return ShikimoriTestRunner.live_delay_ms()

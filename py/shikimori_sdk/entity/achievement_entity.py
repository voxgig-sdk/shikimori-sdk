...
from shikimori_sdk.shikimori_types import (
    Achievement,
    AchievementListMatch,
)
...
class AchievementEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return AchievementEntity(self._client, opts)
    ...
    def match_get(self) -> Achievement:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Achievement]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Achievement().list() works with no args.
        if reqmatch is None:
            reqmatch = {}

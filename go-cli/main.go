// Command Shikimori-cli is a CLI + REPL for the Shikimori SDK,
...
const prompt = "shikimori"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "achievement anime"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      SHIKIMORI_APIKEY (api key), SHIKIMORI_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: SHIKIMORI_APIKEY carries the API key and
	// SHIKIMORI_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("SHIKIMORI_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("SHIKIMORI_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewShikimoriSDK(opts)

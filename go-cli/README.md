...
boru-driven command-line client **and** interactive REPL for the Shikimori
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/shikimori-cli)
make build
...
./shikimori-cli list achievement
./shikimori-cli list anime

# 5. Override the API base URL for a single call
SHIKIMORI_BASE=https://api.example.com ./shikimori-cli list achievement

# 6. No arguments -> interactive REPL
./shikimori-cli
shikimori> list achievement
shikimori> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/shikimori-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/shikimori-cli list achievement
   ```
...
Run with no arguments to open a REPL (prompt `shikimori>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `achievement`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/shikimori-cli`. |
...
(`./shikimori-cli <expr>`) and interactively (the REPL), and expressions compose the

...
The Ruby SDK for the Shikimori API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Achievement` — with named operations (`list`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
raise a `ShikimoriError` (a `StandardError` subclass), so wrap
...
| `progress` | `Integer` | Progress towards next level |
...
| `episodes_aired` | `Integer` | Number of aired episodes |
...
| `myanimelist_id` | `Integer` | MyAnimeList ID |
...
```
rb/
├── Shikimori_sdk.rb       -- Main SDK module

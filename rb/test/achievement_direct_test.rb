...
class AchievementDirectTest < Minitest::Test
  def test_direct_list_achievement
    setup = achievement_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-achievement", setup[:live] ? "live" : "unit")
    if _should_skip
...
def achievement_direct_setup(mockres)

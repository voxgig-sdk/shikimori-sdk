...
module ShikimoriUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ShikimoriResponse.new({})
    ...
    if fetch_err
      ...
      response = ShikimoriResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ShikimoriResponse.new(fetched)
    else

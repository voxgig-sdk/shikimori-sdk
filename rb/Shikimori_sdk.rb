...
require_relative 'Shikimori_types'
...
class ShikimoriSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ShikimoriHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ShikimoriHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ShikimoriFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ShikimoriUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ShikimoriHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ShikimoriHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ShikimoriHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => ShikimoriError.new(
        "#{op}_allow",
        "ShikimoriSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = ShikimoriHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ShikimoriError.new(
        "graphql_error", "ShikimoriSDK: graphql: #{msg}")
      res["graphql"] = errors
    end

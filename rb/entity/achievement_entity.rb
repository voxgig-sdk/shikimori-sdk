...
class AchievementEntity
  ...
  def make
    opts = @_entopts.dup
    AchievementEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ShikimoriHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = ShikimoriHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [AchievementListMatch, Hash, nil] match filter (any subset of
  #   Achievement fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Achievement>, Array] the matching Achievement items; raises ShikimoriError on failure

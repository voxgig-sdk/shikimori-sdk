...
from projectname_sdk.core.control import ShikimoriControl
...
class ShikimoriContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, ShikimoriSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, ShikimoriResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, ShikimoriResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return ShikimoriError(code, msg, self)

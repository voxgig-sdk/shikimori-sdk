...
class FtCtrl extends ShikimoriControl
{
...
class FtClient
{
    ...
    private ShikimoriUtility $utility;

    public function __construct(ShikimoriUtility $utility, array $options)
    {
    ...
    public function get_utility(): ShikimoriUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ShikimoriContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ShikimoriContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ShikimoriContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ShikimoriConfig::make_feature($name);
    ...
    private static function buildUrl(ShikimoriSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ShikimoriError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ShikimoriResponse($fetched);
        }
    ...
    private function fail(ShikimoriContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ShikimoriContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

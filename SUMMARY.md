...
Results: Successful response with list of animes. Returns N+1 results if next page exists when requesting N elements.
...
| --- | --- | --- | --- |
| Achievement | `list` | `GET /achievements` | Required |
| Anime | `list` | `GET /animes` | Required |

## Connect to the API

- Production server: `https://shikimori.one/api`

The default credential is sent in the `Authorization` header with the `Bearer` prefix.

OAuth2 authentication. See https://shikimori.one/oauth for details. User-Agent header with your OAuth2 Application name is required.
...
- `shikimori_list`: List records for an entity. Supported entities: `achievement`, `anime`.
- `shikimori_load`: Load one record for an entity. No active entity supports this operation.

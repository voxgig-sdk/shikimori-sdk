...
import (
	...
	omni "github.com/voxgig-sdk/shikimori-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ShikimoriError); is {
				out := map[string]any{
					"name":    "ShikimoriError",
...
func omniCtx(arg any, client *sdk.ShikimoriSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ShikimoriSDK); is {
			client = live

...
func TestAchievementEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Achievement(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Achievement(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		achievementRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.achievement")))

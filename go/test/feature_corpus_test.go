...
//
// NewShikimoriSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.ShikimoriSDK {
	res, _ := kase["res"].([]any)
	...
	return sdk.NewShikimoriSDK(opts)
}
...
func fcCandidates(client *sdk.ShikimoriSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.ShikimoriSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.ShikimoriSDK, name string) any {

...
class ShikimoriNetsimFeature extends ShikimoriBaseFeature
{
    ...
    private function _simulate(ShikimoriContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(ShikimoriContext $ctx, array $applied): void
    {
        $client = $this->client;

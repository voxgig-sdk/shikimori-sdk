...
{
    private static function utility(): ShikimoriUtility
    {
        return new ShikimoriUtility();
    }
    ...
    private static function ctx(array $over = []): ShikimoriContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ShikimoriContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ShikimoriOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ShikimoriError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ShikimoriOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ShikimoriMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ShikimoriOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ShikimoriMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ShikimoriOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ShikimoriMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ShikimoriSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ShikimoriMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new ShikimoriResponse([]),
            'result' => new ShikimoriResult([]),
        ]);
        [, $err] = ShikimoriMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new ShikimoriSpec([]),
            'response' => null,
            'result' => new ShikimoriResult([]),
        ]);
        [, $err] = ShikimoriMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ShikimoriSpec(['step' => 's']),
            'response' => new ShikimoriResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ShikimoriResult([]),
        ]);
        [, $err] = ShikimoriMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ShikimoriSpec(['step' => 's']),
            'response' => new ShikimoriResponse(self::resp(200, ['v' => 1])),
            'result' => new ShikimoriResult([]),
        ]);
        [, $err] = ShikimoriMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ShikimoriSpec(['step' => 's']),
            'response' => new ShikimoriResponse(self::resp(200, ['v' => 2])),
            'result' => new ShikimoriResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ShikimoriMakeResponse::call($ctx);
    ...
    {
        $preset = new ShikimoriResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ShikimoriSpec([]),
            'response' => new ShikimoriResponse([]),
            'result' => new ShikimoriResult([]),
        ...
        [$got, $err] = ShikimoriMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ShikimoriResult([])]);
        [, $err] = ShikimoriMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ShikimoriSpec([]), 'result' => null]);
        [, $err] = ShikimoriMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ShikimoriSpec(['step' => 's']),
            'result' => new ShikimoriResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ShikimoriOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ShikimoriMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ShikimoriSpec(['step' => 's']),
            'result' => new ShikimoriResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ShikimoriOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ShikimoriMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ShikimoriResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ShikimoriSpec([]),
            'result' => new ShikimoriResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ShikimoriMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ShikimoriMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ShikimoriError('boom', 'boom');
        $utility->fetcher = function (ShikimoriContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ShikimoriSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ShikimoriMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ShikimoriContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ShikimoriMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ShikimoriResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ShikimoriContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ShikimoriSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ShikimoriMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ShikimoriResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ShikimoriSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ShikimoriMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ShikimoriMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ShikimoriMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ShikimoriResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ShikimoriDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ShikimoriResult(['ok' => false])]);
        $this->expectException(ShikimoriError::class);
        ShikimoriDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ShikimoriResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ShikimoriMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        ShikimoriMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ShikimoriBaseFeature();
        $b = new ShikimoriBaseFeature();
        ShikimoriFeatureAdd::call($ctx, $a);
        ShikimoriFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ShikimoriBaseFeature
    {
        $f = new ShikimoriBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ShikimoriFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): ShikimoriContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ShikimoriSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ShikimoriPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ShikimoriResponse(['headers' => null]),
            'result' => new ShikimoriResult([]),
        ]);
        ShikimoriResultHeaders::call($ctx);

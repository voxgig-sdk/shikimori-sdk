...
// Shikimori SDK Achievement entity
...
class AchievementEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new AchievementEntity($this->_client, $opts);
    }

    /**
     * @param Achievement|array $args Achievement data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ShikimoriHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Achievement|array The current Achievement data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Achievement fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ShikimoriHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Achievement fields).
     */
     ...
     *
     * @param AchievementListMatch|array|null $reqmatch Match filter (any subset
     *   of Achievement fields) as an assoc-array; AchievementListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Achievement[]|array A list of Achievement items as assoc-arrays at
     *   the SDK boundary; throws ShikimoriError on failure (item-5 convention).
     */

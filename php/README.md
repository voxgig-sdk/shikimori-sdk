...
The PHP SDK for the Shikimori API — an entity-oriented client using PHP conventions.
...
print_r(array_map(fn($item) => $item->data_get(), $achievement));
```
...
```
php/
├── shikimori_sdk.php          -- Main SDK class
...
The main class (`shikimori_sdk.php`) exports the SDK class

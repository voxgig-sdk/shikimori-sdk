...
The Lua SDK for the Shikimori API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Achievement()` — each with the same small set of operations (`list`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
| `id` | `number` | Anime ID |
| `image` | `table` |  |
| `japanese` | `table` | Japanese names |
| `kind` | `string` | Anime type |
| `myanimelist_id` | `number` | MyAnimeList ID |
| `name` | `string` | Anime name |
| `ongoing` | `boolean` | Is ongoing |
| `rates_scores_stats` | `table` | Rating statistics |
| `rates_statuses_stats` | `table` | Status statistics |
...
```
lua/
├── shikimori_sdk.lua    -- Main SDK module
...
The main module (`shikimori_sdk`) exports the SDK constructor

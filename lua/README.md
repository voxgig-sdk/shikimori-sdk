...
The Lua SDK for the Shikimori API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Achievement(nil):list(nil, nil)
...
local result, err = client:Shikimori(nil):load(
  { id = "test01" }, nil
)

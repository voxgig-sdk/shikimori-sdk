...
local ShikimoriSDK = {}
ShikimoriSDK.__index = ShikimoriSDK
...
ShikimoriSDK._make_feature = _make_feature


function ShikimoriSDK.new(options)
  local self = setmetatable({}, ShikimoriSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function ShikimoriSDK:options_map()
...
function ShikimoriSDK:get_utility()
  return Utility.copy(self._utility)
end


function ShikimoriSDK:get_root_ctx()
  return self._rootctx
end


function ShikimoriSDK:prepare(fetchargs)
...
function ShikimoriSDK:direct(fetchargs)
...
-- by passing it.
function ShikimoriSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function ShikimoriSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "ShikimoriSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function ShikimoriSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = ShikimoriSDK.new(sdkopts)
  sdk.mode = "test"

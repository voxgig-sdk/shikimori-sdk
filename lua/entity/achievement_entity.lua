...
local AchievementEntity = {}
AchievementEntity.__index = AchievementEntity


function AchievementEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AchievementEntity)
  self._name = "achievement"
  self._client = client
...
function AchievementEntity:get_name()
  return self._name
end


function AchievementEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AchievementEntity.new(self._client, opts)
end
...
function AchievementEntity:mark_deleted()
  self._deleted = true
end


function AchievementEntity:deleted()
  return true == self._deleted
end


function AchievementEntity:data_set(args)
  if args ~= nil then
...
function AchievementEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AchievementEntity:match_set(args)
  if args ~= nil then
...
function AchievementEntity:match_get()
...
function AchievementEntity:stream(action, args, callopts)
...
function AchievementEntity:_run_op(ctx, post_done)
  local utility = self._utility

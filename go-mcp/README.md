...
[MCP](https://modelcontextprotocol.io) server exposing the Shikimori SDK as
two agent tools — `shikimori_list` and `shikimori_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/shikimori-mcp)
make build

# 2. Provide credentials via the environment
export SHIKIMORI_APIKEY=sk_live_xxx
...
```jsonc
// shikimori_list: first page of records
{ "entity": "achievement" }
{ "entity": "achievement", "query": { } }

// shikimori_load: one record by id
{ "entity": "achievement", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/shikimori-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user shikimori \
     -- "$PWD"/dist/*/shikimori-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `shikimori_list` and `shikimori_load` tools now appear
   in new sessions. Ask the agent to *"list achievement using shikimori"*
   and it calls `shikimori_list` with `{"entity":"achievement"}`.
...
```sh
export SHIKIMORI_APIKEY=sk_live_xxx            # API key
export SHIKIMORI_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `shikimori_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `shikimori_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `SHIKIMORI_APIKEY` | API key sent with every request. |
| `SHIKIMORI_BASE` | Optional override of the API base URL. |
